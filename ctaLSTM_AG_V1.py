...
########################################################################
class ctaLSTM_AG_V1(CtaTemplate):
    className = 'ctaLSTM_AG_V1'
    ...
    # ----------------------------------------------------------------------
    def __init__(self, ctaEngine, setting, filepath='/home/chocolate/LSTM-source/models/model_ag_1/predict.ckpt'):
        """Constructor"""
        super(ctaLSTM_AG_V1, self).__init__(ctaEngine, setting)
        self.DATAS = []
...
#         saver.restore(sess_1, self.filepath)
    ...
    def onTick(self, tick):
        ...
        if tickMinute != self.barMinute:
            if self.bar:
                # print u' '
                # print u'当前时间', tick.datetime
                # self.writeCtaLog(str(tick.datetime))
                ...
                self.onBar(self.bar)  # 在每次tick.minute更新时将上一分钟的bar数据推送给onbar

            bar = CtaBarData()   # ctaBase.py中的类
...
# 对买一价和卖一价保持和tick中同样的名称，方便后续调用进行建仓/平仓
        ...
        if self.count >= 15:
            ...
            self.writeCtaLog(u'class:' + str(self.predict))
            ...
            if self.pos == 0:
                self.kclose_set()
                # 当前持仓为0，考虑停止5分钟再进行建仓判断,防止过于频繁交易

                # 得分大于买多阈值时，建多仓
                if self.predict == 1 and self.kclose_label == False:
            ...
            if self.pos != 0:
                self.kopen_set()
                # 当前若持仓，时间超过8分钟后再交易，否则瞬间交易，盈利不足手续费
...
# 创建会话，加载模型
    ...
    def long_pos_sell(self, long_pos):  # 多仓的平仓考虑。
        if self.predict == -1 and self.kopen_label == False:
            self.sell(self.bidprice1, 1)
            self.pos_rec -= 1
        ...
        elif self.rec_price - self.bidprice1 >= self.zhisun * self.rec_price:  # 止损
            self.sell(self.bidprice1, 1)
            self.pos_rec -= 1
            self.writeCtaLog(u'止损平多仓:' + u'sell_Price' +
                             str(long_pos.bidPrice1))
            self.kclose_label = True
            self.kclose_bar = 0
            self.zhisun -= self.zhisun * 0.06
            self.zhiying -= self.zhiying * 0.03

        elif self.bidprice1 - self.rec_price >= self.zhiying * self.rec_price:  # 止盈
            self.sell(self.bidprice1, 1)
            self.pos_rec -= 1
            self.writeCtaLog(u'止盈平多仓:' + u'sell_Price' +
                             str(long_pos.bidPrice1))
            self.kclose_label = True
            self.kclose_bar = 0
            self.zhiying += self.zhiying * 0.03
    ...
    def short_pos_cover(self, short_pos):  # 持有空仓时的平仓考虑
        ...
        elif self.askprice1 - self.rec_price >= self.zhisun * self.rec_price:  # 止损
            self.cover(self.askprice1, 1)
            self.pos_rec += 1
            self.writeCtaLog(u'止损平空仓' + u'cover_Price' +
                             str(short_pos.askPrice1))
            self.kclose_label = True
            self.kclose_bar = 0
            self.zhisun -= self.zhisun * 0.06
            self.zhiying -= self.zhiying * 0.03

        elif self.rec_price - self.askprice1 >= self.zhiying * self.rec_price:   # 止盈
            self.cover(self.askprice1, 1)
            self.pos_rec += 1
            self.writeCtaLog(u'止盈平空仓:' + u'cover_Price' +
                             str(short_pos.askPrice1))
            self.kclose_label = True
            self.kclose_bar = 0
            self.zhiying += self.zhiying * 0.03
...
# 因此设置zhisun_label作为是否能重新建仓的标志，以zhisun_bar作为停止的分钟时长的计数。
    ...
    def kclose_set(self):
        if self.kclose_label is True:
            self.kclose_bar += 1
        if self.kclose_bar >= 10:
            self.kclose_bar = 0
            self.kclose_label = False

    def kopen_set(self):
        if self.kopen_label is True:
            self.kopen_bar += 1
        if self.kopen_bar >= 8:
            self.kopen_bar = 0
            self.kopen_label = False
    ...
    def vi_mem_record(self):
        ...
        # vms虚拟内存mem_info.total, mem_info.percent)  # vms虚拟内存
        return (psutil.Process(os.getpid()).memory_info().vms, psutil.Process(os.getpid()).memory_info().rss, mem_info.total, mem_info.percent)
