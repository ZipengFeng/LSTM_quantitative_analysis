...
########################################################################
class ctaLSTM_V6_1(CtaTemplate):
    className = 'ctaLSTM_V6_1'
    ...
    # ----------------------------------------------------------------------
    def __init__(self, ctaEngine, setting, filepath='/home/chocolate/LSTM-source/models/model_7/predict_EMA.ckpt'):
        """Constructor"""
        super(ctaLSTM_V6_1, self).__init__(ctaEngine, setting)
        self.DATAS = []
...
# 对买一价和卖一价保持和tick中同样的名称，方便后续调用进行建仓/平仓
        ...
        if self.count >= 15:
            ...
            self.predict = self.pred(seq_data)
            self.writeCtaLog(u'class:' + str(self.predict))
            # self.predict_records.append([bar.datetime, self.predict])

            # if self.count >= 16:
            #     if self.predict == 3:
            #         self.count_correct += 1
            #     elif self.predict == 1:
            #         if datafr['Dis_EMA5_26'][-1] > 0 & datafr['Dis_EMA5_26'][-1] > datafr['Dis_EMA5_26'][-2]:
            #             self.count_correct += 1
            #     elif self.predict == 2:
            #         if datafr['Dis_EMA5_26'][-1] > 0 & datafr['Dis_EMA5_26'][-1] < datafr['Dis_EMA5_26'][-2]:
            #             self.count_correct += 1
            ...
            #     count_correct_f = float(self.count_correct)
            #     self.acc = count_correct_f / (count_f - 15.0)
            ...
            if self.pos == 0:
                ...
                if self.predict == 4 and self.zhisun_label == False:
