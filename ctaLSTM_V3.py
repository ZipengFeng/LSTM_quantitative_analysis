...
########################################################################
class ctaLSTM_V3(CtaTemplate):
    className = 'ctaLSTM_V3'
    ...
    def __init__(self, ctaEngine, setting, filepath=None):
        """Constructor"""
        super(ctaLSTM_V3, self).__init__(ctaEngine, setting)
        self.DATAS = []
...
# 对买一价和卖一价保持和tick中同样的名称，方便后续调用进行建仓/平仓
        ...
        if self.count >= 26:
            ...
            if self.pos == 0:
                ...
                # 得分大于买多阈值时，建多仓
                if self.predict_1 >= 5 and self.score >= self.buy_raise_score:
                ...
                # 得分小于买空阈值时，建空仓
                if self.predict_1 <= 3 and self.score <= self.buy_down_score:
...
# 创建会话，加载模型
        ...
        saver.restore(sess_1, path + "/predict_1min.ckpt")
        saver.restore(sess_2, path + "/predict_2min.ckpt")
        saver.restore(sess_3, path + "/predict_3min.ckpt")
        saver.restore(sess_4, path + "/predict_4min.ckpt")
        saver.restore(sess_5, path + "/predict_5min.ckpt")
...
# 7：  4：小涨   3：平稳   2：小跌   1：大跌
    ...
    def long_pos_sell(self, long_pos):  # 多仓的平仓考虑。
        if self.predict_1 <= 3 and self.score <= self.sell_down_score:
            self.sell(self.bidprice1, 1)
            self.pos_rec -= 1
            self.writeCtaLog(u'sell_Price' + str(long_pos.bidPrice1))
            # self.zhisun_label = True
    ...
    def short_pos_cover(self, short_pos):  # 持有空仓时的平仓考虑。
        if self.predict_1 >= 5 and self.score >= self.sell_raise_score:
            self.cover(self.askprice1, 1)
            self.pos_rec += 1
            self.writeCtaLog(u'cover_Price' + str(short_pos.askPrice1))
