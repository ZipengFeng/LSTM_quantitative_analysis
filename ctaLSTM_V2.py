...
########################################################################
class ctaLSTM_V2(CtaTemplate):
    className = 'ctaLSTM_V2'
    ...
    def __init__(self, ctaEngine, setting, filepath=None):
        """Constructor"""
        super(ctaLSTM_V2, self).__init__(ctaEngine, setting)
        self.DATAS = []
        self.records = []
        self.datas = []
        self.lastOrder = None
    ...
    def onStop(self):
        ...
        self.saveFile = file(path + 'datas.csv', 'wb')
        self.writer = csv.writer(self.saveFile)
        self.writer.writerows(self.datas)
        self.saveFile.close()

        self.saveFile = file(path + 'records.csv', 'wb')
        self.writer = csv.writer(self.saveFile)
    ...
    def onTick(self, tick):
        ...
        if tickMinute != self.barMinute:
            ...
            if self.pos > 0 and self.pos_rec > 0:
                self.long_pos_sell(tick)
                # self.long_back_wave()##需要考虑如果两个函数都平仓了如何解决的问题
            if self.pos < 0 and self.pos_rec < 0:
                self.short_pos_cover(tick)
                # self.short_back_wave()
    ...
    def onBar(self, bar):
        ...
        new_data = {'close': bar.close, 'max': bar.high, 'min': bar.low,
                    ...
                    'askpr1': bar.askPrice1, 'askvo1': bar.askvo1,
...
# 对买一价和卖一价保持和tick中同样的名称，方便后续调用进行建仓/平仓
        ...
        if bar.datetime.hour == 14 and bar.datetime.minute == 58:
            ...
            if self.pos > 0:
                ...
                self.records.append([bar.datetime, self.price, u'sell'])
            if self.pos < 0:
                self.cover(self.askprice1, 1)
                self.zhisun_label = True
                self.records.append([bar.datetime, self.price, u'cover'])
        ...
        if self.count >= 6:
            ...
            if self.pos == 0:
                ...
                if self.score > self.buy_raise_score and self.zhisun_label is False:
                    ...
                    self.records.append([bar.datetime, self.price, u'buy'])
                ...
                if self.score < self.buy_down_score and self.zhisun_label is False:
                    ...
                    self.records.append([bar.datetime, self.price, u'short'])
...
# 5：大涨   4：小涨   3：平稳   2：小跌   1：大跌
    ...
    def long_pos_sell(self, long_pos):##多仓的平仓考虑。
        if long_pos.bidPrice1>self.rec_price:##做多的时候，实时价格高于对比价格，更新对比价格，止损系数，止盈系数。
            self.zhisun_ying_rate_update(long_pos)
            self.writeCtaLog(u'多仓更新价格'+str(self.rec_price)+u'止损比例'+str(1-self.zhisun)+u'止盈比例'+str(1+self.zhiying))#更新的价格，止损，止盈。
        ...
        if long_pos.bidPrice1 < (1 - self.zhisun)*self.rec_price:#止损策略
            ...
            self.zhisun_label = True
            self.zhisun_bar = 0
            self.records.append([long_pos.datetime,self.price,u'sell'])
    ...
    def short_pos_cover(self, short_pos):##持有空仓时的平仓考虑。
        if short_pos.askPrice1 < self.rec_price:
            self.zhisun_ying_rate_update(short_pos)
            self.writeCtaLog(u'空仓更新价格'+str(self.rec_price)+u'止损比例'+str(1+self.zhisun)+u'止盈比例'+str(1-self.zhiying))#更新的价格，止损，止盈。
        ...
        if short_pos.askPrice1 > (1+self.zhisun)*self.rec_price: ##做空的时候，实时价格高于止损线
            ...
            self.zhisun_label = True
            self.zhisun_bar = 0
            self.records.append([short_pos.datetime,self.price,u'cover'])
    ...
    def zhisun_ying_rate_update(self, own_pos):
        ...
        if self.zhisun > 0:
            self.zhisun = self.zhisun - 0.000008
        if self.zhiying > 0:
            self.zhiying = self.zhiying + 0.000008#tick中和bar中更新止盈止损一致

    def zhisun_set(self):#每次止损平仓后，考虑停止5分钟进行操作，因此设置zhisun_label作为是否能重新建仓的标志，以zhisun_bar作为停止的分钟时长的计数。
